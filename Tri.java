...
class Tri {
	...
	public static void main(String[] args) {
		...
		if (nouveauNombre == 0) {System.out.println("Désolé il y a eu un erreur, n'entrez pas \"0\".");} else {
			...
			while (nouveauNombre != 0) {
				...
				if (nouveauNombre != 0) {listeDesNombres.add(nouveauNombre);}
			}
			...
			for (int i = 0; i < listeDesNombres.size() - 1; i++) {
				...
				if (listeDesNombres.get(i) == listeDesNombres.get(i + 1)) {constant += 1;}
				if (listeDesNombres.get(i) <= listeDesNombres.get(i + 1)) {croissant += 1;}
				if (listeDesNombres.get(i) >= listeDesNombres.get(i + 1)) {decroissant += 1;}
			}
			...
			if (listeDesNombres.size() > 1) {
				...
				for (int nombre : listeDesNombres) {
					if (nombre > plusGrand) {
						...
						plusGrand = nombre;
					} else if (nombre > dexiemePlusGrand) {dexiemePlusGrand = nombre;}
					if (nombre < plusPetit) {
						dexiemePlusPetit = plusPetit;
						plusPetit = nombre;
					} else if (nombre < dexiemePlusPetit) {dexiemePlusPetit = nombre;}
				}
