...
class DeckOfCards {
	...
	DeckOfCards(Card card) {this.addCard(card);} // create a deck from a single card (Card object)
	...
	// redefines the set of the deck

	void shuffle() {for (int i = 0; i < this.getSize() * 3; i++) {this.addCard(this.pickRandom());}}
	// shuffle the deck by picking random cards inside

	ArrayList <Card> getDeck() {return this.deck;}
	// get the deck as an ArrayList

	void addDeck(DeckOfCards newDeck) {this.deck.addAll(newDeck.getDeck());}
	...
	Card getCard(int index) {return this.deck.get(index);}

	int getRank(int index) {return this.deck.get(index).getRank();}
